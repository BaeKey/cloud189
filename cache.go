...
// Resolve 将一个 WebDAV 路径逐级解析到最终 Entry。
// 这里保留极短 TTL 的元数据缓存，专门削减 rclone 挂载时的重复探测成本。
...
// List 返回目录下一级子项列表，优先命中极短 TTL 缓存，其次做并发去重。
...
func (c *PathCache) listDir(ctx context.Context, client *Cloud189Client, dirPath string, dirID string) (cacheDir, error) {
	...
	loaded := newCacheDir(items, c.ttl)
...
// newCacheDir 由目录列表构造临时索引结构，供极短时间内的重复访问复用。
